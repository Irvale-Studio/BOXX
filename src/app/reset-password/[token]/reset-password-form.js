...
export default function ResetPasswordForm({ token, tenantId }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background" />}>
      <ResetPasswordFormInner token={token} tenantId={tenantId} />
    </Suspense>
  )
}

function ResetPasswordFormInner({ token, tenantId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/auth/reset-password', {
        ...
        body: JSON.stringify({ email, token, password, tenantId: tenantId || undefined }),
      })
