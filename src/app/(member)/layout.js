...
function SocialLinks({ theme }) {
  ...
  const links = [
    ...
    { key: 'social_line', label: 'LINE', icon: (
  ...
  const activeSocials = links.filter(l => theme[l.key])
  ...
  return (
    <div className="flex items-center gap-3">
      {activeSocials.map(({ key, label, icon }) => (
        <a
          key={key}
          href={key === 'social_line' ? (theme[key].startsWith('http') ? theme[key] : `https://line.me/R/ti/p/${theme[key]}`) : theme[key]}
          target="_blank"
