...
const features = [
  {
    ...
    detail: 'Led by coaches certified in British boxing standards',
  },
  {
    id: 'intimate',
    number: '02',
    ...
    detail: 'Small groups for maximum personal attention',
  },
  {
    id: 'luxury',
    number: '03',
    ...
    detail: 'Boutique studio with imported equipment',
  },
  {
    id: 'inclusive',
    number: '04',
    ...
    description:
      'Whether you\'ve never thrown a punch or you\'re preparing for a fight — there\'s a place for you at BOXX. Everyone starts somewhere.',
    detail: 'From complete beginners to experienced boxers',
  },
];
...
function FeatureCard({ feature, index, isActive, onHover }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex-1 min-w-0 border-l transition-all duration-700 cursor-default ${
        isActive
          ? 'border-accent flex-[2.5] pl-8 pr-6 py-8'
          : 'border-white/10 hover:border-white/20 flex-1 pl-6 pr-4 py-8'
      }`}
    >
      {/* Number */}
      <span
      ...
      {/* Title */}
      <h3
        className={`text-xl md:text-2xl font-bold tracking-tight mt-4 transition-colors duration-500 ${
        ...
        <div className="md:hidden space-y-6">
          {features.map((feature, i) => (
            <motion.div
              ...
              className="border-l border-accent/30 pl-6 py-2"
            >
              <span className="text-xs tracking-widest text-accent">{feature.number}</span>
              <h3 className="text-xl font-bold mt-2">{feature.title}</h3>
              <p className="text-white/50 text-sm leading-relaxed mt-2">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
